    ...
    , ui(new Ui::MainWindow)
{
    ...
    { //初始化网络
        ...
        connect(ui->slider_b, &QSlider::valueChanged, this, &MainWindow::on_brightnessChanged);
        connect(ui->slider_c, &QSlider::valueChanged, this, &MainWindow::on_colorChanged);
        connect(ui->slider_s, &QSlider::valueChanged, this, &MainWindow::on_satChanged);
        on_pushButton_3_clicked();
        ui->slider_b->setValue(100);
        ui->slider_c->setValue(hue);
        ui->slider_s->setValue(sat);
        ui->label_2->setText("Бабуиний свет");
    }
}
...
{
    while (udp_socket.hasPendingDatagrams()) { // 是否有待处理的信号
...
void MainWindow::on_brightnessChanged(int value)
{
    int pos = ui->slider_b->value();
    QString slider_value = QString("%1").arg(pos) + "%";
    ui->percent_b->setText(slider_value);
...
void MainWindow::on_colorChanged(int value)
{
    hue = ui->slider_c->value();
    QString slider_value = QString("%1").arg(hue);
    ui->percent_c->setText(slider_value);
    setHsv();
}

void MainWindow::on_satChanged(int value)
{
    sat = ui->slider_s->value();
    QString slider_value = QString("%1").arg(sat);
    ui->percent_s->setText(slider_value);
    setHsv();
}
...
void MainWindow::setHsv()
{
    ...
    if (bulb.size() > 0) {
        ...
        cmd_str->append(QString("%1, ").arg(hue));
        cmd_str->append(QString("%1, ").arg(sat));
        cmd_str->append(", \"smooth\", 500]}\r\n");
        tcp_socket.write(cmd_str->data());
        qDebug() << cmd_str->data();
    } else {
