...
public:
    ...
    int updateClientPoints(Leet::ID userID, int points)
    {
        m_clients[userID]->setPoints(m_clients[userID]->getPoints() + points);
    }
...
private:
    ...
    void addToContestList(Level diff, Leet::ID id)
    {
        switch (diff) {
        case Level::LOW:
            contestsLow.push_back(id);
            return;
        case Level::MEDIUM:
            contestsMedium.push_back(id);
            return;
        case Level::HIGH:
            contestsHigh.push_back(id);
            return;
        default:
            return;
        }
    ...
    void addToQuestionList(Level diff, Leet::ID id)
    {
        switch (diff) {
        case Level::LOW:
            questionsLow.push_back(id);
            return;
        case Level::MEDIUM:
            questionsMedium.push_back(id);
            return;
        case Level::HIGH:
            questionsHigh.push_back(id);
            return;
        default:
            return;
        }
